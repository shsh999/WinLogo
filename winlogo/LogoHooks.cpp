...
inline HTHEME g_logoThemeHandle = nullptr;
...
HTHEME newOpenThemeDataForDpi(decltype(OpenThemeDataForDpi) original, HWND hwnd,
    ...
    if (_wcsicmp(pszClassList, LOGO_THEME_NAME) == 0) {
        if (g_logoThemeHandle != nullptr && g_logoThemeHandle != newHandle) {
            OutputDebugStringA("Warning: taskbarpearl opened more than once!");
        }
        g_logoThemeHandle = newHandle;
    }
    return newHandle;
}

HRESULT newCloseThemeData(decltype(CloseThemeData) original, HTHEME hTheme) {
    if (g_logoThemeHandle == hTheme) {
        g_logoThemeHandle = nullptr;
    }
...
HRESULT newEndBufferedPaint(decltype(EndBufferedPaint) original, HPAINTBUFFER hBufferedPaint,
    ...
    if (g_logoThemeHandle && g_logoDisabledImage && targetWindow == findStartButton()) {
